...
const themes = {
    ...
    anime: animeImages,
    animals: [...surprisedAnimalImgs, ...capybaraImgs]
};

// DOM loads --> listen for a button click 
document.addEventListener('DOMContentLoaded', () => {
const button = document.getElementById('button');

// one-time request every time button is clicked
button.addEventListener('click', () => {
  // first check for valid input 
  const userInp = getInput(); 
  if (validTheme(userInp)) {
    // call async func 
    sendTheme(userInp).catch((errors) => {alert('Errors encountered sending mssg to app.js: ' + errors);});   // will be alerted of error 
  } else {
        document.getElementById('error-mssg').style.visibility = 'visible';
        setTimeout(() => {
          document.getElementById("theme").value = '';  // clear form
          document.getElementById('error-mssg').style.visibility = 'hidden';  // hide message again
        }, 2000);
      }
});
...
function sendTheme(userInp) {
  let queryOpts = {active: true, currentWindow: true};
  chrome.tabs.query(queryOpts, (tab) => {
    // send message thru chrome API method
    chrome.tabs.sendMessage(
      tab[0].id,
      {memes: themes[userInp]},
      function (response) {
